...
INTEGRATION_CONTAINER_NAME="cl_central-ledger-int"
...
function runMigration() {
  docker exec -it ${INTEGRATION_CONTAINER_NAME} sh -c "npm run migrate"
}
...
function runTests() {
  ...
  docker exec -it ${INTEGRATION_CONTAINER_NAME} sh -c "npm run test:int"
}

function copyResults() {
  echo "Copying results from: ${JEST_JUNIT_OUTPUT_DIR}/${JEST_JUNIT_OUTPUT_NAME} to: ${RESULTS_DIR}"
  docker cp ${INTEGRATION_CONTAINER_NAME}:${JEST_JUNIT_OUTPUT_DIR}/${JEST_JUNIT_OUTPUT_NAME} ${RESULTS_DIR}
}
